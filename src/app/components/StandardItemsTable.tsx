...
export default function StandardItemsTable({ items, onChange }: StandardItemsTableProps) {
  ...
  return (
    <div className="mb-8">
      ...
      {/* Datalist for SKU autocomplete */}
      <datalist id="sku-list">
        {skuOptions.map(s => (
          <option key={s.skuCode} value={s.skuCode}>{s.skuName}</option>
        ))}
      </datalist>
              ...
              <tr key={item.id} className="border-b border-border last:border-0 row-hover">
                <td className="px-4 py-2">
                  <input
                    type="text"
                    list="sku-list"
                    placeholder="Enter or select SKU number"
                ...
                <td className="px-4 py-2">
                  ...
                  >
                    ...
                    {uomOptions.length > 0
                      ? uomOptions.map(u => (
                          <option key={u.id} value={u.uomCode}>{u.uomCode} — {u.uomName}</option>
                        ))
                      : ['PCS', 'BOX', 'CARTON', 'UNIT'].map(u => (
